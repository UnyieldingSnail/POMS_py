...
DATA_PATH = os.path.expanduser("~\\data\\data-net\\")
OUT_PATH = os.path.expanduser("~\\data\\out\\")

LABEL_NUM = 1

graphs_train = {}
with open(OUT_PATH + 'random_walks_train.txt', 'r', encoding='utf8') as f:
    for line in f:
...
graphs_val = {}
with open(OUT_PATH + 'random_walks_val.txt', 'r', encoding='utf8') as f:
    for line in f:
        walks = line.strip().split('\t')
        graphs_val[walks[0]] = []
        for i in range(1, len(walks)):
...
graphs_test = {}
with open(OUT_PATH + 'random_walks_test.txt', 'r', encoding='utf8') as f:
    for line in f:
        walks = line.strip().split('\t')
        graphs_test[walks[0]] = []
        for i in range(1, len(walks)):
...
labels_train = {}
sizes_train = {}
with open(DATA_PATH + 'cascade_train.txt', 'r', encoding='utf8') as f:
    for line in f:
        profile = line.split('\t')
...
labels_val = {}
sizes_val = {}
with open(DATA_PATH + 'cascade_val.txt', 'r', encoding='utf8') as f:
    for line in f:
        profile = line.split('\t')
...
labels_test = {}
sizes_test = {}
with open(DATA_PATH + 'cascade_test.txt', 'r', encoding='utf8') as f:
    for line in f:
        profile = line.split('\t')
...
pickle.dump((x_data, y_data, sz_data, index.length()), open(OUT_PATH + 'data_train.pkl', 'wb'))
...
pickle.dump((x_data, y_data, sz_data, index.length()), open(OUT_PATH + 'data_val.pkl', 'wb'))
...
pickle.dump((x_data, y_data, sz_data, index.length()), open(OUT_PATH + 'data_test.pkl', 'wb'))

np.random.seed(13)
with open(OUT_PATH + 'node_vec_50.txt', 'r') as f:
    line = f.readline()
...
pickle.dump(node_vec, open(OUT_PATH + 'node_vec.pkl', 'wb'))
