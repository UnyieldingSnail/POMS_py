...
from main.model import DeepPOMS

tf.set_random_seed(0)

NUM_THREADS = 20
MODEL_PATH = os.path.expanduser("~\\data\\out\\")
...
x_train, y_train, sz_train, vocabulary_size = pickle.load(open(MODEL_PATH + 'data_train.pkl', 'rb'))
x_test, y_test, sz_test, _ = pickle.load(open(MODEL_PATH + 'data_test.pkl', 'rb'))
x_val, y_val, sz_val, _ = pickle.load(open(MODEL_PATH + 'data_val.pkl', 'rb'))
node_vec = pickle.load(open(MODEL_PATH + 'node_vec.pkl', 'rb'))
...
start = time.time()
model = DeepPOMS(config, sess, node_vec)
step = 1
best_val_loss = 1000
best_test_loss = 1000
# 训练
