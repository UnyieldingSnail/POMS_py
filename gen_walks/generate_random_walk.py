...
import time
from gen_walks.node2vec import Graph
...
op.add_option("--data_root", dest="data_root", type="string", default="~\\data",
              help="data root.")
op.add_option("--dataset", dest="dataset", type="string", default="data-net",
              help="data set.")
op.add_option("--out", dest="out", type="string", default="out",
              help="out.")
...
out_path = os.path.join(opts.data_root, opts.out)
...
graph_walk_prefix = os.path.join(out_path, "random_walks_")
# Output file, word2vec向量。
embed_prefix = os.path.join(out_path, "node_vec_")
sets = ["train", "val", "test"]
...
def get_global_info():
    rfile = open(global_graph_file, 'r', encoding='utf8')
    for line in rfile:
        line = line.rstrip('\r\n')
        parts = line.split("\t\t")
        source = long(parts[0])
...
def parse_graph(graph_string):
    ...
    # 图id\t[作者id]\t时间\t节点数量\t[源节点:目标节点:权重]\t[标签]
    ...
    first_time = True
    G = Graph(nx_G, True, opts.p, opts.q)
...
def read_graphs(which_set):
    ...
    write_file = open(graph_walk_file, 'w', encoding='utf8')
    rfile = open(graph_file, 'r', encoding='utf8')
...
def read_walks_set(which_set, walks):
    graph_walk_file = graph_walk_prefix + which_set + ".txt"
    rfile = open(graph_walk_file, 'r', encoding='utf8')
    for line in rfile:
        line = line.rstrip('\r\n')
